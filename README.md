...
$$
h_l(t+1)=h_l(t)+\Delta t\left[-h_l(t)+\rho_l\left(W_l h_{l-1}(t)+\gamma W_{l+1}^{\top}h_{l+1}(t)+b_l\right)\right].
$$

The output state evolves according to

$$
y(t+1)=y(t)+\Delta t\left[-y(t)+\rho_{\mathrm{out}}\left(W_{\mathrm{out}}h_{\mathrm{last}}(t)+b_{\mathrm{out}}\right)\right].
$$
...
$$
\Delta W_l=\frac{\eta_l}{B}\sum_{n=1}^{B}\left(m_{l,n}\odot h_{l,n}\right)h_{l-1,n}^{\top}.
$$
