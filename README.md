...
\rho_{\mathrm{out}}
\left(
W_{\mathrm{out}} h_{\mathrm{last}}(t)
+
b_{\mathrm{out}}
\right)
\right].
...
All right-hand-side states are evaluated at the same time step, so the updates are synchronous.
...
\[
a \sim \mathrm{Categorical}(\mathrm{logits}=y).
\]
...
and hidden-layer modulators are propagated through reciprocal transpose feedback connections,

\[
m_l = W_{l+1}^{\top}m_{l+1}.
\]

The local synaptic update is based on presynaptic activity, postsynaptic activity, and the propagated modulator,
...
DCF uses the same free-equilibrium dynamics, reciprocal feedback pathway, and local update rule, but replaces the reward-based output signal with
...
The main paper reports results over six random seeds. A simple shell loop can be used to reproduce them. For example:
...
Please add the license appropriate for your intended release (for example, MIT, BSD-3-Clause, or another license required by your institution).
