...
# ============================================================
# Direct Cost-Feedback Equilibrium Learning for CNNs
# ============================================================
#
# Controlled single-phase variant used to test direct propagation
# of the full supervised output cost signal:
#
#         C        = 1/2 ||y - t||^2
#         M_output = -dC/dy = t - y
...
#
# The reciprocal convolutional equilibrium dynamics are identical
# to the NEL CNN implementation: max-pooling, max-unpooling, and
# transpose-convolution feedback with tied forward kernels.
#
# There is NO action selection, NO exploration, NO reward signal,
# NO nudged phase, NO contrastive EP update, and NO activation
...
def build_parser():
    parser = argparse.ArgumentParser(
        description="Direct Cost-Feedback Equilibrium Learning for convolutional networks"
    )
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "attached NeurIPS-2019 implementation). 'normal' uses N(0, init_scale^2)."
        ),
    )
    ...
    parser.add_argument(
        "--save-path",
        type=str,
        default="./results_direct_cost_feedback_cnn",
...
# ============================================================
# Direct Cost-Feedback CNN model
# ============================================================


class DirectCostFeedbackEquilibriumCNN(nn.Module):
    """
    Reciprocal equilibrium CNN trained by direct full-output cost feedback.
    ...
    def _pool_bottom_up(self, x: torch.Tensor, conv_states: Sequence[torch.Tensor]):
        ...
        the pooling operation in top-down dynamics and in the local update.
        """
        pooled_drives = []
        pool_indices = []
        prepool_sizes = []
    ...
    def output_cost_signal(
        ...
        if not torch.isfinite(output).all():
            ...
            raise RuntimeError(
                "Non-finite output encountered before cost-feedback update "
    ...
    def update_weights(
        ...
        # ========================================================
        output_mod = self.output_cost_signal(
            output,
            labels,
        )
        ...
        for i in range(self.num_conv):
            ...
            # Same direct cost-feedback post-factor as in the MLP implementation.
            ...
            # postsynaptic cost-feedback factor. This is the convolutional analogue of:
...
def build_run_name(args, model):
    ...
    return (
        f"DirectCostFeedbackCNN_{args.task}"
        f"_ch{channels_str}"
        f"_fc{fc_str}"
...
def save_results(
    ...
    train_acc_array = np.asarray(train_acc, dtype=np.float64)
...
def main():
    ...
    # Model
    # --------------------------------------------------------
    model = DirectCostFeedbackEquilibriumCNN(
    ...
    # --------------------------------------------------------
    print("=" * 84)
    print("Direct Cost-Feedback Equilibrium Learning - Convolutional Network")
    print("=" * 84)
    ...
    train_acc = [chance_accuracy]
    test_acc = [chance_accuracy]
    epoch_time = []

    if args.save:
        os.makedirs(save_path, exist_ok=True)
        save_results(save_path, train_acc, test_acc, epoch_time, config)
    ...
    for epoch in range(args.epochs):
        ...
        train_acc.append(100.0 * run_acc)
        test_acc.append(100.0 * test_acc_t)
        epoch_time.append(elapsed)

        print(
            "Epoch {:03d} | Train Acc: {:.2f}% | Test Acc: {:.2f}% | Time: {:.1f}s".format(
                epoch + 1,
                100.0 * run_acc,
                100.0 * test_acc_t,
                elapsed,
